...
exports.deleteAllStudentsInClass = async (req, res) => {
  const { classId } = req.params;
  ...
  try {
    ...
    try {
      ...
      const feeDeleteRes = await FeedingFeeRecord.deleteMany({ student: { $in: studentIds } }).session(session);
      const attendanceDeleteRes = await StudentAttendance.deleteMany({ student: { $in: studentIds } }).session(session);

      await session.commitTransaction();
      session.endSession();

      if (enrollmentController && enrollmentController.clearEnrollmentCache) {
        enrollmentController.clearEnrollmentCache(req.user.school);
      }

      return res.json({
        message: `Successfully deleted ${studentIds.length} students from the class.`,
        deletedCount: studentIds.length,
        deletedFeedingFees: feeDeleteRes.deletedCount,
        deletedAttendances: attendanceDeleteRes.deletedCount
      });
    } catch (txErr) {
      console.error('Error deleting students (transaction attempt failed):', txErr);
      if (session) {
      ...
      const feeDeleteRes = await FeedingFeeRecord.deleteMany({ student: { $in: studentIds } });
      const attendanceDeleteRes = await StudentAttendance.deleteMany({ student: { $in: studentIds } });
      ...
      return res.json({
        message: `Successfully deleted ${studentIds.length} students from the class (fallback).`,
        deletedCount: studentIds.length,
        deletedFeedingFees: feeDeleteRes.deletedCount,
        deletedAttendances: attendanceDeleteRes.deletedCount,
        note: 'Fallback deletion used.'
      });
    }
  } catch (err) {
    console.error('❌ Error in deleteAllStudentsInClass:', err);
